...
def train_episodes(env, agent, n_test, goal, n_episodes=500):
...
def main():
    ...
    agent = DQN(env.observation_space.shape, env.action_space.n, log_dir,
                FCFeatures([20, 20]),
                OriginalQLayer([10]),
                RandomReplay(10000, 200),
                DoubleDQN(),
                2E-3, 0.4, 0, 0.95, 2000)
    agent.save_hyperparameters()
    rewards, aver_rewards = train_episodes(env, agent, N_TEST, GOAL, MAX_EPISODES)
